...
https://github.com/moskomule/senet.pytorch/blob/master/baseline.py
"""
...
class ResNet(nn.Module):
    def __init__(self, block, n_size, num_classes=10, input_features=16):
        super(ResNet, self).__init__()
        self.inplane = input_features
        self.conv1 = nn.Conv2d(3, self.inplane, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.inplane)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, input_features, blocks=n_size, stride=1)
        self.layer2 = self._make_layer(block, input_features * 2, blocks=n_size, stride=2)
        self.layer3 = self._make_layer(block, input_features * 4, blocks=n_size, stride=2)
